...
- **常用菜单热键**：`F1` 冒险之证、`F3` 祈愿和 `F5` 活动菜单映射到移动端顶部按钮；键鼠宏可保留 `F1`–`F8` 虚拟键事件，已配置的菜单键会执行触控，其余键保持安全空操作。
- **PC 默认键位语义**：显式覆盖 `B` 背包、`C` 角色、`X` 下落、`Enter` 聊天、`Y` 通知详情、`U` 特殊环境信息、`G` 教程详情与 `T` 情境交互，避免把 DeviceHub profile 的任意触发字母误当成原神 PC 键位。
- **虚拟鼠标拖拽**：`moveMouseTo` 会维护脚本虚拟光标；左键按住期间的 `moveMouseTo`/`moveMouseBy` 在释放时合并为一次触控拖拽，支持列表、滑条等社区脚本操作且不增加截图请求。`setGameMetrics` 同步缩放 1080p/2K/4K 输入坐标。
...
- 一键战斗宏：首次调用 JS host `oneKeyFight.KeyDown()` 时，会把
...
| GameLoading 自动开门（进入/顶号确认/适龄提示/月卡与原石弹窗） | ✅ 离线链路，待真机门页复验 |
