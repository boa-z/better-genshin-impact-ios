...
SUPPORTED = [
    ...
    "BvPage", "BvLocator", "BvFlow", "BvFlowAction", "Flow", "Run",
    "WithDefaultTimeout", "WithDefaultRetryInterval", "WithTimeout",
    "WithRetryInterval", "WaitUntil", "WaitUntilAny", "WaitUntilText",
    "WaitUntilAnyText", "WaitUntilDisappear", "WaitUntilAllDisappear",
    "Until", "UntilAny", "UntilText", "UntilAnyText", "UntilDisappear",
    "UntilAllDisappear", "OpenCvSharp.Rect", "getVersion",
    ...
    "moveMouseBy", "moveMouseTo", "verticalScroll", "rightButton", "middleButton", "keyMouseScript", "inputText", "http.request", "genshin.returnMainUi",
    "genshin.switchParty", "genshin.switchCharacter",
    ...
    "genshin.blessingOfTheWelkinMoon", "genshin.goToAdventurersGuild",
    "genshin.goToCraftingBench", "genshin.goCraftResin",
    "genshin.craftMaterial", "genshin.setTime", "genshin.wonderlandCycle",
    "pathingScript",
    ...
    "dispatcher.runAutoWoodTask",
    "dispatcher.runQuickSereniteaPotTask", "dispatcher.runQuickClaimRewardTask",
    "dispatcher.runCheckRewardsTask",
    "dispatcher.runWalkToFTask", "dispatcher.runScanPickTask",
    "dispatcher.runLowerHeadThenWalkToTask",
    "dispatcher.runBlessingOfTheWelkinMoonTask",
    "dispatcher.runClaimBattlePassRewardsTask",
    "dispatcher.runClaimEncounterPointsRewardsTask",
    "dispatcher.runClaimMailRewardsTask",
    "dispatcher.runGoToAdventurersGuildTask",
    "dispatcher.runGoToCraftingBenchTask", "dispatcher.runGoCraftResinTask",
    "dispatcher.runCraftMaterialTask", "dispatcher.runSetTimeTask",
    "dispatcher.runWonderlandCycleTask", "dispatcher.runReloginTask",
    "dispatcher.runChooseTalkOptionTask",
    "dispatcher.runLinneaMiningTask",
    "dispatcher.runQuickBuyTask",
    "dispatcher.runOneKeyExpeditionTask",
    "dispatcher.runAutoTrackTask",
...
PARTIAL = {
    ...
    "dispatcher.runTask": "已支持 奖励/协会导航/合成/调时/QuickBuy/UseRedemptionCode/AutoArtifactSalvage/CountInventoryItem/GetGridIcons/InventoryCountComparison/CharacterDevelopment/OneDragon/ScriptGroup/Shell；AutoBoss/地脉已内置官方路线，幽境已内置活动导航状态机",
    ...
    "genshin.switchCharacter": "iOS 端使用队伍配置页 OCR 重组指定角色槽位，并按单机/联机玩家可控槽位映射；需要可用 OCR 和已导入的 Genshin profile",
    ...
    "dispatcher.runWalkToFTask": "已迁移前进至交互提示；iOS 使用 F 模板/OCR 和 DeviceHub W/冲刺映射",
    "dispatcher.runScanPickTask": "已迁移掉落扫描；默认复用共享 AutoPick 触发器，安装 bgi_world 模型后可启用目标框精确靠近",
    "dispatcher.runLowerHeadThenWalkToTask": "已迁移追踪点低头靠近；需要对应模板资产或显式 targetMatName",
    "genshin.commonJobs": "奖励、协会/合成台路线、材料合成和调时已接入 iOS；路线/按钮识别仍需对应账号与真机画面回归",
